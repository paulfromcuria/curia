...
- **Manchester and Cheshire only.** Do not research or suggest anything in Los Angeles, Riyadh, Santorini, London, Chicago, or any other metro. This is explicit, current user direction, not a placeholder — re-check it every run even if it feels restrictive. `docs/data/venues.json` is genuinely multi-metro now (six real markets as of 2026-09-18, not just an aspiration) — don't let a metro's real presence in that file read as permission to touch it.
- **Districts to place venues in**: read `docs/data/districts.json` for the current real list, **filtered to `metro: 'manchester'` or `metro: 'cheshire'`** — don't hardcode a list here, it grows, and don't just take "every district in the file" at face value now that Hyde Park, Kenwood, Woodlawn, Riyadh's and Santorini's and London's districts all live in the same file. See "District suggestions" below for when and how to propose adding to it.
- **All three categories** (Do/Drink/Eat) matter over time. Before researching, read `docs/data/venues.json` and `docs/data/tiles.json` and do a quick manual tally: for each real onboarding tile (`docs/data/tiles.json`), count how many current **Manchester/Cheshire** venues (real + still-`pending`/`approved` in `venue-suggestions.json`) actually match it by type — filter to the district list above first, then count, don't tally the whole file. **Real, found 2026-09-18**: several tile-matching types now exist only outside Manchester/Cheshire (e.g. CAFETERIA/CARIBBEAN RESTAURANT/SENEGALESE/DINER/SOUL FOOD, all Chicago-only, all matching the real 'Neighbourhood favourite' tile) — an unfiltered whole-file count would make that tile look better-covered for Manchester/Cheshire than it actually is, and quietly steer research away from a real local gap. Bias today's research toward whichever category/district combination is thinnest **within Manchester/Cheshire specifically** — the same "coverage gap" idea the admin dashboard's Growth Insights panel computes (`src/lib/admin/dashboard-insights.ts`), just done by hand here since you can't run the live app.
