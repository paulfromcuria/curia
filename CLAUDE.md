...
- Doesn't run on a normal daily/weekly schedule at all (`passesRegularScheduleFilter` / `Venue.occasional`, 2026-09-22, direct user report: Wilmslow Artisan Market — a real monthly street market, its own curated copy already said "once a month... check the date" — was being recommended on an ordinary Tuesday). `bands`/`openingHours` both assume a schedule that repeats every day or every week; neither can express "the third Saturday of the month," so this is a separate hard filter, not an extension of the two above. Defaults to `false`/unset for the whole catalog (a normal venue) — only ever set `true` by deliberate curation for a venue whose real-world schedule this data model genuinely cannot represent (a market, a pop-up, a seasonal event space).
- Booking required — a member genuinely cannot walk in without arranging it first (`passesWalkInFilter` / `Venue.bookingRequired`, 2026-09-22, same audit pass, at direct user follow-up "do we have other venues similar to that"). Distinct from `occasional` above: White Peak Alpaca Farm ("pre-booked walks only") likely does run a normal weekly schedule — the problem isn't when it's open, it's that showing up unannounced doesn't work regardless. Same defaults-`false`, only-set-on-explicit-copy-evidence discipline as `occasional` — never inferred from venue type alone.
...
- Weather gets the same treatment for its one genuinely decisive case: `scoreWeatherFitFactor` (2026-09-19, found by re-auditing for the same bug class right after the fix above) multiplies an outdoor-only venue's score by ×0.4 during genuinely extreme weather (a real storm, not "a bit cold") — weather's own additive weight, even at its existing x3 extreme-weather boost, could only ever separate an ideal indoor pick from a poor outdoor one by about 8 points out of 100. Weather still drives `scoreWeather`/`contextNoteFor` exactly as before for everything short of genuinely extreme conditions.
