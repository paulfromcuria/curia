...
- Tile match, sub-preference match, spend level, time of day (`BAND_MULT`), day of week (`DAY_MULT`), district liveliness (`bandMultiplier` — the district's real-time "how alive is it here" curve, distinct from a venue's own opening hours), weather, pet fit (see note below), proximity (added 2026-09 — a light "closer wins, all else equal" tiebreaker *inside* the hard radius; the radius cutoff itself is unchanged).
